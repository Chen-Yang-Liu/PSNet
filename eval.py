...
def save_captions(args, word_map, hypotheses, references):
    ...
    with open('eval_results_fortest/' + args.Split +'/'+args.encoder_image + "_" +args.encoder_feat+"_" + '_res.json', 'w') as f:
        json.dump(result_json_file, f)

    with open('eval_results_fortest/' + args.Split +'/'+ args.encoder_image + "_" +args.encoder_feat+"_" + '_gts.json', 'w') as f:
...
def evaluate_transformer(args,encoder_image,encoder_feat,decoder):
    ...
    with torch.no_grad():
        for i, (image_pairs, caps, caplens, allcaps) in enumerate(
            ...
            encoder_out = encoder_out.expand(S,k, encoder_dim,3)  # [S,k, encoder_dim]
            encoder_out = encoder_out.permute(1,0,2,3)
            ...
            while True:
                ...
                encoder_out = encoder_out.permute(1, 0, 2,3)
                pred = decoder.caption_decoder(tgt_embedding, encoder_out, tgt_mask=mask)  # (length, batch, feature_dim)
                encoder_out = encoder_out.permute(1, 0, 2,3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default="./data2/",help='folder with data files saved by create_input_files.py.')
    ...
    parser.add_argument('--encoder_image', default="vit_b_32") # inception_v3 or vgg16 or vgg19 or resnet50 or resnet101 or resnet152
    ...
    # parser.add_argument('--checkpoint', default="./models_checkpoint_GRSL/BEST_checkpoint_",help='model checkpoint.')
    parser.add_argument('--model_path', default="./models_checkpoint/data/1-times/", help='model checkpoint.')
    args = parser.parse_args()

    # for beam_size in [1]:  # ,,(0,6),(2,2),
    ...
    for i in range(len(filename)):
        ...
        checkpoint_path = os.path.join(args.model_path, filename[i])
        print(args.model_path + filename[i])
